...
//*---------------------------------------------* 
	if(handle_outset_client)
		if(!(client_list->check_client_to_stdin(socket_fd, outset_client, client_id, outset_fd)))
			return EXEC_ERROR;
//*---------------------------------------------* 
	if(handle_destination_client)
		if(!(client_list->check_stdout_to_client(socket_fd, client_id, destination_client, destination_fd)))
			return EXEC_ERROR;
//*---------------------------------------------* 
	int stat;	
	pid_t exec_pid = fork();	
//*---------------------------------------------* 
	if(exec_pid == 0)
	{
		switch(pipe_type)
		{
			case PIPE_SINGLE:
				if(handle_outset_client)
					client_list->redirect_client_to_stdin(outset_fd);
				else
					pipe_vector[0]->redirect_pipe_to_stdin();
//*---------------------------------------------* 
				if(handle_destination_client)
					client_list->redirect_stdout_to_client(destination_fd);
...
//*---------------------------------------------* 
	...
	{
		if(handle_outset_client)
			;//client_list->close_client_to_stdin(outset_fd);	
		else
...
//*---------------------------------------------* 
		switch(pipe_type)	
		{
			case PIPE_SINGLE:
				if(handle_destination_client)
					;//client_list->close_stdout_to_client(destination_fd);	
				else if(handle_output_file)
...
//*---------------------------------------------* 	
	if(handle_outset_client)
		client_list->unlink_client_to_stdin(outset_fd, outset_client, client_id, command_string);	
	if(handle_destination_client)
		client_list->broacast_stdout_to_client(destination_fd, client_id, destination_client, command_string);	
//*---------------------------------------------* 	
...
//*---------------------------------------------*/
			client_list->tell(socket_fd, client_id, transfer_id, command_string);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
	}
...
//*---------------------------------------------*/ 
			client_list->yell(socket_fd, client_id, command_string);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
	}
