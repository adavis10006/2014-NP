...
//*---------------------------------------------* 
	if(handle_destination_client)
	{
		...
		sem_signal(sem_id);
		while(!client_list_check_stdout_to_client_fd(socket_fd, client_id, destination_client, destination_fd));	
	}
//*---------------------------------------------* 
	int stat;	
	pid_t exec_pid = fork();	
...
//*---------------------------------------------* 
			sem_wait(sem_id);
			client_list_tell(socket_fd, client_id, transfer_id, message.c_str());
			sem_signal(sem_id);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
...
//*---------------------------------------------* 
			sem_wait(sem_id);
			client_list_yell(socket_fd, client_id, message.c_str());
			sem_signal(sem_id);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
