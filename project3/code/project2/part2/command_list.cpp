...
//***********************************************
COMMAND_LIST::COMMAND_LIST(int client_id, int socket_fd): client_id(client_id), socket_fd(socket_fd), command_string(NULL), environment_list()
{
...
//*---------------------------------------------*
	for(int i = 0; i < strlen(command_string); i++)
		if(command_string[i] == '\r' || command_string[i] == '\n')
			command_string[i] = '\0';
//*---------------------------------------------*
...
//***********************************************
PARSE_RESULT COMMAND_LIST::parse_argv(PIPE_TYPE& pipe_type, int& exec_argc, int& remove_argc, int& pipe_destination, bool& handle_destination_client, int& destination_client, bool& handle_outset_client, int& outset_client, bool& handle_new_line, bool& handle_output_file, bool& handle_error)
{
	exec_argc = -1;
	remove_argc = -1;
	pipe_destination = 1;
	destination_client = -1;
	outset_client = -1;
	handle_destination_client = false;
	handle_outset_client = false;
...
//*---------------------------------------------*   
					...
					{
						exec_argc = exec_argc == -1 ? i : exec_argc;
						remove_argc = i + 1;
//*---------------------------------------------*   
...
//*---------------------------------------------*   
				...
				else if(sscanf(command_list[i]->command, ">%d", &destination_client) == 1)
				{
					exec_argc = exec_argc == -1 ? i : exec_argc;
					remove_argc = i + 1;
//*---------------------------------------------*   
					handle_destination_client = true;
				}
				else
				{
					error("pipe format", false);
//*---------------------------------------------*   
				...
				break;
			case '<':
				if(sscanf(command_list[i]->command, "<%d", &outset_client) == 1 && (pipe_type == PIPE_SINGLE || pipe_type == PIPE_FRONT))
				{
					exec_argc = exec_argc == -1 ? i : exec_argc;
					remove_argc = i + 1;
//*---------------------------------------------*   
					handle_outset_client = true;
				}
				else
				{
					error("pipe format", false);
...
//*---------------------------------------------*   
	if((handle_destination_client && handle_new_line) || (handle_destination_client && handle_output_file) || (handle_output_file && handle_new_line))
		return PARSE_ERROR;
//*---------------------------------------------*   
	switch(pipe_type)
	{
		case PIPE_SINGLE:
			break;
		case PIPE_FRONT:
			if(handle_new_line || handle_destination_client || handle_output_file)
				return PARSE_ERROR;
			break;
		case PIPE_MIDDLE:
			if(handle_outset_client || handle_new_line || handle_destination_client || handle_output_file)
				return PARSE_ERROR;
			break;
		case PIPE_END:
			if(handle_outset_client)
				return PARSE_ERROR;
			break;
	}	
//*---------------------------------------------*   
...
//***********************************************
EXEC_RESULT COMMAND_LIST::exec_command(PIPE_TYPE pipe_type, int pipe_destination, bool handle_destination_client, int destination_client, bool handle_outset_client, int outset_client, bool handle_new_line, bool handle_output_file, bool handle_error, int output_file_fd, char **argv, bool& exec_error)
{
	int outset_fd, destination_fd;
	exec_error = false; 
//*---------------------------------------------* 
...
//*---------------------------------------------* 
	...
	{
		sem_wait(sem_id);
		if(!(client_list_check_client_to_stdin(socket_fd, outset_client, client_id, outset_fd)))
		{
			sem_signal(sem_id);
			return EXEC_ERROR;
		}
...
//*---------------------------------------------* 
	if(handle_destination_client)
	{
		sem_wait(sem_id);
		if(!(client_list_check_stdout_to_client(socket_fd, client_id, destination_client, destination_fd)))
		{
			sem_signal(sem_id);
			return EXEC_ERROR;
		}
	...
	if(exec_pid == 0)
	{
		switch(pipe_type)
		{
			case PIPE_SINGLE:
				...
				{
					sem_wait(sem_id);
					client_list_redirect_client_to_stdin(outset_fd);
					sem_signal(sem_id);
				}
				else
...
//*---------------------------------------------* 
				...
				{
					sem_wait(sem_id);
					client_list_redirect_stdout_to_client(destination_fd);
					sem_signal(sem_id);
				}
...
//*---------------------------------------------* 
	...
	{
		if(handle_outset_client)
			;//client_list_close_client_to_stdin(outset_fd);	
		else
...
//*---------------------------------------------* 
		switch(pipe_type)	
		{
			case PIPE_SINGLE:
				if(handle_destination_client)
					;//client_list_close_stdout_to_client(destination_fd);	
				else if(handle_output_file)
...
//*---------------------------------------------* 	
	...
	{
		sem_wait(sem_id);
		client_list_unlink_client_to_stdin(outset_fd, outset_client, client_id, command_string);	
		sem_signal(sem_id);
	}
	if(handle_destination_client)
	{
		sem_wait(sem_id);
		client_list_broacast_stdout_to_client(destination_fd, client_id, destination_client, command_string);	
		sem_signal(sem_id);
	}
//*---------------------------------------------* 	
...
//*---------------------------------------------* 	
		char **argv;	
		int command_argc, exec_argc, remove_argc, pipe_destination, destination_client, outset_client;
		bool handle_destination_client, handle_outset_client, handle_new_line, handle_output_file, handle_error;
		int output_file_fd;
//*---------------------------------------------* 			
		if(parse_argv(pipe_type, exec_argc, remove_argc, pipe_destination, handle_destination_client, destination_client, handle_outset_client, outset_client, handle_new_line, handle_output_file, handle_error) == PARSE_ERROR)
		{
			std::string error_input("Input format error.\n");
//*---------------------------------------------* 
			safety_write(socket_fd, error_input.c_str(), strlen(error_input.c_str()));
			return COMMAND_ERROR;
		}
//*---------------------------------------------* 
		debug_parse_argv(pipe_type, exec_argc, remove_argc, pipe_destination, handle_destination_client, destination_client, handle_outset_client, outset_client, handle_new_line, handle_output_file, handle_error);
//*---------------------------------------------* 
...
//*---------------------------------------------* 
		if((exec_result = exec_command(pipe_type, pipe_destination, handle_destination_client, destination_client, handle_outset_client, outset_client, handle_new_line, handle_output_file, handle_error, output_file_fd, argv, exec_error)) == EXEC_ERROR)
		{
...
//*---------------------------------------------*
		delete(argv);
//*---------------------------------------------* 
		if(remove_argc == -1 || exec_error || exec_result == EXEC_FORBID || exec_result == EXEC_ERROR)
...
//*---------------------------------------------* 
		...
		{
			sem_wait(sem_id);
			client_list_who(socket_fd, client_id);
			sem_signal(sem_id);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
...
//*---------------------------------------------*/ 
			sem_wait(sem_id);
			client_list_tell(socket_fd, client_id, transfer_id, command_string);
			sem_signal(sem_id);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
...
//*---------------------------------------------*/ 
			sem_wait(sem_id);
			client_list_yell(socket_fd, client_id, command_string);
			sem_signal(sem_id);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
...
//*---------------------------------------------* 
		...
		{
			sem_wait(sem_id);
			client_list_name(socket_fd, client_id, command_list[1]->command);
			sem_signal(sem_id);
		}
//*---------------------------------------------* 
		return COMMAND_SUCCESS;
...
void debug_parse_argv(PIPE_TYPE pipe_type, int exec_argc, int remove_argc, int pipe_destination, bool handle_destination_client, int destination_client, bool handle_outset_client, int outset_client, bool handle_new_line, bool handle_output_file, bool handle_error)
{
	...
	printf("[SERV]|PAGV| destination_client: %d\n", destination_client);
	printf("[SERV]|PAGV| outset_client: %d\n", outset_client);
	printf("[SERV]|PAGV| handle_destination_client: %s\n", handle_destination_client ? "TRUE" : "FALSE");
	printf("[SERV]|PAGV| handle_outset_client: %s\n", handle_outset_client ? "TRUE" : "FALSE");
