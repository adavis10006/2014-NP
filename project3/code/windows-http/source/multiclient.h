...
struct CLIENT 
{
    ...
    CLIENT(const char *input_host,const int input_port, const char *input_batch): 
        port(input_port)
    {
        host = new char[strlen(input_host)];
        batch = new char[strlen(input_batch)];
...
{
public:
    MULTICLIENT(int socket_fd, const char *query_string, const char *temp_exec_path);
    
    bool parse_query(const char *query_string);
