...
//*---------------------------------------------*    
    while((pid = waitpid(getpid(), &stat, WNOHANG)) > 0)
		;//printf("client kill pid %d\n", pid);
}
//***********************************************
void sig_chld_read(int signo)
{
    pid_t pid;
    int stat;
//*---------------------------------------------*    
...
//***********************************************
void debug_sig_chld_read()
{
#ifdef DEBUG_SIGNAL
	printf("[SERV]|SIGN| Signal SIGCHLD handler->sig_chld_read\n");
#endif
}
//***********************************************
