...
bool parse_request( int source_socket, SOCKS4_REQUEST *request )
{
    ...
    {
        request->user_id = new char[ read_length + 1 ];
    ...
    if( mask_ip( request ) )
    {
        ...
        if( ( read_length = read( source_socket, socket_buffer, sizeof( socket_buffer ) ) ) <= 0 )
        {
            error_handle( "READ REQUEST DOMAIN NAME ERROR\n" );
            return false;
        }
        else
        {
            request->domain_name = new char[ read_length + 1 ];
            strcpy( request->domain_name, socket_buffer );
        }
    }
    return true;
}
...
void SOCKS4::LISTEN()
{
    ...
    debug_handle( "VN: %d CD: %d SRC IP: %s PROT: %s DST IP: %s PORT: %s\n", request.VN, request.CD, source_ip, source_port, destination_ip, destination_port );
    ...
    request.user_id = reply.user_id = NULL;
    if( request.domain_name != NULL )
    {
        delete [] request.domain_name;
        request.domain_name = reply.domain_name = NULL; 
    }
}
//***********************************************
void SOCKS4::REDIRECT( int destination_socket )
{
    ...
    bool SRC2DST_finish = false, SRC2DST_done = false, DST2SRC_finish = false, DST2SRC_done = false;
    char buffer[ 20 + 1 ];
    ...
    while( true )
    {
        ...
        if( !DST2SRC_done && DST2SRC_write_limit != 0 )
        {    
            FD_SET( source_socket, &write_fds );
            set_fds = true;
        }
        if( !SRC2DST_done && SRC2DST_write_limit != 0 )
        {    
