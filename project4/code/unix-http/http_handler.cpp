...
void HTTP_HANDLER::response()
{
    ...
    if(http_response.response_type != RESPONSE_OK)
    {
        ...
        if(strcmp(http_analysis.file_type, "cgi") == 0)
        {
            ...
            if(signal(SIGCHLD, sig_chld_cgi) == SIG_ERR)
            ...
            if(exec_pid == 0)
            {
                ...
                extern char **environ;  
                char *argv[2] = {http_analysis.request_file, NULL};
                environ = http_analysis.header.get_vector();   
                execvp(http_analysis.request_file, argv);

                exit(EXEC_ERROR);
            }
            else if(exec_pid < 0)
                error_handle("cgi: exec error in fork\n");

            waitpid(-1, &stat, 0);

            if(WIFEXITED(stat) && WEXITSTATUS(stat) == EXEC_ERROR)
                error_handle("cgi: exec error in exec\n");
        }
        else
        {
